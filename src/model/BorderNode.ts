...
class BorderNode extends Node implements IDropTarget {
    ...
    public static _fromJson(json: any, model: Model): BorderNode {
    ...
    public _layoutBorderInner(inner: Rect) {
        ...
        const split2 =
            DockLocation.split(
                DockLocation.reflect(location.name).name,
                split1.start,
    ...
    public drop(dragNode: (Node & IDraggable), _location: DockLocation.DockLocation, index: number): void {
        let fromIndex = 0;
        ...
        if (parent !== undefined) {
            ...
            } else if (parent instanceof BorderNode && parent.getSelected() !== -1) {
